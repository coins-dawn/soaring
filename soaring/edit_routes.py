...
def main(
    ...
    spot_to_refpoints_list = read_json(input_spot_to_refpoints_path, "spot_to_refpoints")
    spot_to_spots_list = read_json(input_spot_to_stops_path, "spot_to_stops")
    stop_to_refpoints_list = read_json(input_stop_to_refpoints_path, "stop_to_refpoints")
    ...
    for elem in merged_list:
        ...
        keypair_to_duration_dict[(from_key, to_key)] = duration
        
    with open(output_all_routes_path, "w", encoding="utf-8") as f:
        f.write("from,to,duration_m\n")
        for (from_key, to_key), duration_m in keypair_to_duration_dict.items():
            f.write(f"{from_key},{to_key},{duration_m}\n")
