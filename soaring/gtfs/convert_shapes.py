...
def get_shape_between_stops(
    stop_a, stop_b, stop_locations, trip_stops, trip_shapes, shapes
):
        ...
        if stop_a in stop_indices and stop_b in stop_indices:
            if stop_indices[stop_a] < stop_indices[stop_b]:
                ...
                if shape_id and shape_id in shapes:
                    ...
                    index_a = find_nearest_shape_point(
                        lat_a, lon_a, shape_points, stop_a, trip_id
                    )
                    index_b = find_nearest_shape_point(
                        lat_b, lon_b, shape_points, stop_b, trip_id
                    )

                    if index_a < index_b:
                        shape_segment = [
                            (lat, lon)
                            for _, lat, lon in shape_points[index_a : index_b + 1]
                        ]
...
def process_stop_pairs(
    input_file, output_json, stops_file, stop_times_file, trips_file, shapes_file
):
    ...
    with open(input_file, newline="", encoding="utf-8-sig") as csvfile:
        ...
        for i, row in enumerate(reader, start=1):
            ...
            stop_to = row["stop_to"]
            shape = get_shape_between_stops(
                stop_from, stop_to, stop_locations, trip_stops, trip_shapes, shapes
            )
            if shape:
                results.append(
                    {"stop_from": stop_from, "stop_to": stop_to, "shape": shape}
                )
